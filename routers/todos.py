...
async def search_todos(query: str, user, db: Session = Depends(get_db)):
    todos = db.query(models.Todos).filter(
        models.Todos.title.ilike("%{}%".format(query)),
        models.Todos.owner_id == user.get('id')).all()
    return todos
...
@router.get("/search/")
async def search(
        request: Request, user: user_dependency,
        db: Session = Depends(get_db),
        query: Optional[str] = None):
    todos = await search_todos(query, db=db, user=user)
    return templates.TemplateResponse(
